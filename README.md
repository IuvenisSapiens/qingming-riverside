...
- 点击水面：产生波纹。
- 声音图标：开启或关闭水声、划桨声和饮茶杯声，默认关闭。动画暂停或页面进入后台后静音。
...
`world.js` 管理行人往返与渡船状态；乘船包含连续上下台阶、缓慢离岸、横渡、减速靠岸和返程。`scene.js` 负责画面、角色动作、镜头、命中区域与输入。`sound.js` 使用 Web Audio 合成声音，首次点击声音开关才创建音频上下文；不请求外部音频。
